...
function PlayerSwingSwordState:init(player, area)
    -- init player
    self.player = player
    -- init area
    self.area = area
    ...
    -- directions the player is faing
    ...
    -- init hitbox. Ie, when the sword touches hitbox, the monster takes damage
...
function PlayerSwingSwordState:enter(params)
    -- sounds would go here if you had any
...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.area.currentRoom.entities) do
        if entity:collides(self.swordHitbox) then
            entity:damage(1)
            -- sound would go here to indicate they were hit IF YOU HAD ONE
        end
    end
    ...
    if love.keyboard.wasPressed('space') then
        self.player:changeState('character-swing-sword')
    end
end
